...
class StoreDiff(object):
    ...
    active_units = cached_property(_active_units)
        ...
        for (tag, i1, i2, j1, j2) in self.opcodes:
            if tag == 'equal':
                dbids = set(self.db_units[uid]['id']
                            ...
                            and not ((self.file_units[uid]['source']
                                      == self.db_units[uid]['source_f'])
                                     and (self.file_units[uid]['target']
                                          == self.db_units[uid]['target_f'])))
                update_dbids.update(dbids)
        return update_dbids

    def diff(self):
        """Return a dictionary of change actions"""
        _diff = {}
        _diff["index"] = self.get_indexes_to_update()
        _diff["obsolete"] = self.get_units_to_obsolete()
        _diff["add"] = self.get_units_to_add()
        _diff["update"] = self.get_units_to_update()
        return _diff
